...
export class PredictionService {
  ...
  async getPredictions(retailerId: number) {
    const history = await this.analyticsRepo.getSalesHistoryForPrediction(retailerId);

    // Group sales by product
    const productSales: { [productId: number]: { name: string; totalQuantity: number; dailySales: { [date: string]: number } } } = {};
    ...
    for (const [productId, data] of Object.entries(productSales)) {
      ...
      if (avgDailyDemand > 3) movementType = 'FAST_MOVING';
      if (avgDailyDemand < 0.5) movementType = 'SLOW_MOVING';
