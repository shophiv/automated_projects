...
export class InventoryRepository {
  ...
  async getHistory(retailerId: number, queryParams: { productId?: string; type?: string; limit?: string; offset?: string }) {
    let query = `
  ...
  async getValuation(retailerId: number) {
    const query = `
      SELECT 
  ...
  async getAlerts(retailerId: number) {
    const query = `
