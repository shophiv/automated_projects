...
class AnalyticsService {
    static async getSalesAnalytics(tenantId, interval = 'daily') {
      ...
      LIMIT 30;
    `;
        const { rows } = await database_1.pool.query(query, [tenantId, dateFormat]);
    ...
    `;
        const { rows: paymentRows } = await database_1.pool.query(paymentQuery, [tenantId]);
    ...
    `;
        const { rows: categoryRows } = await database_1.pool.query(categoryQuery, [tenantId]);
        return {
            sales_trend: rows,
