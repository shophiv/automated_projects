...
class ProductRepository {
    async findAll(tenantId, search, categoryId, limit = 50, offset = 0) {
        ...
        const res = await (0, database_1.query)(queryText, params);
        return res.rows;
    }
    async findById(tenantId, id) {
        const res = await (0, database_1.query)(`SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.tenant_id = $1 AND p.id = $2`, [tenantId, id]);
        return res.rows[0] || null;
    }
    async create(tenantId, data) {
        const res = await (0, database_1.query)(`INSERT INTO products (
    ...
    async update(tenantId, id, data) {
        const res = await (0, database_1.query)(`UPDATE products SET
    ...
    async delete(tenantId, id) {
        const res = await (0, database_1.query)(`DELETE FROM products WHERE tenant_id = $1 AND id = $2 RETURNING id`, [tenantId, id]);
        return res.rows[0] || null;
    }
    async getPricingConfig(tenantId) {
        const res = await (0, database_1.query)(`SELECT * FROM pricing_configurations WHERE tenant_id = $1`, [tenantId]);
        return res.rows[0] || null;
    }
    async upsertPricingConfig(tenantId, globalMargin, categoryMarginsJson) {
        const res = await (0, database_1.query)(`INSERT INTO pricing_configurations (tenant_id, global_margin, category_margins_json, updated_at)
       VALUES ($1, $2, $3, CURRENT_TIMESTAMP)
       ON CONFLICT (tenant_id) 
       DO UPDATE SET global_margin = $2, category_margins_json = $3, updated_at = CURRENT_TIMESTAMP
       RETURNING *`, [tenantId, globalMargin, categoryMarginsJson]);
        return res.rows[0];
    }
}
