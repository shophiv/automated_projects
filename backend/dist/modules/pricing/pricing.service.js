...
class PricingService {
    async configureMargins(retailerId, config) {
      ...
      `;
            await database_1.pool.query(query, [retailerId, config.globalMargin.toString()]);
        }
        if (config.categoryMargins) {
          ...
          DO UPDATE SET setting_value = EXCLUDED.setting_value, updated_at = CURRENT_TIMESTAMP
        `;
                await database_1.pool.query(query, [retailerId, `category_margin_${catId}`, margin.toString()]);
            }
        }
        return { message: 'Pricing margins configured successfully' };
    }
    async getMargins(retailerId) {
        const query = `
    ...
    calculateSellingPrice(purchasePrice, margin) {
